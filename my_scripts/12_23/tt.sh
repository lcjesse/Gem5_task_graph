...
DATE=`date +%m_%d`
# OUT_FILE_DIR=${GEM5}/my_STATS/11_24/multi_app
...
if [ "$1" = "run" ];then
    ...
    do
        ...
        do
            ...
            do
                # vc num for each vnet
                for VC_NUM in ${vc_num[@]}
                do
                    # vc num in vnet for ddr
                    ...
                    do
    {
        if [ "$VC_DDR" -lt "$VC_NUM" ]; then
            ...
            --task-graph-file="${GEM5}/my_benchmarks/11_25/Memory_Access_${MEM_ACCESS}_Memory_Type_${MEM_TYPE}/multi_application_0${APP_NUM}.cfg" \
            --routing-algorithm=2 \
            --vcs-per-vnet=${VC_NUM} \
            --vcs-for-ddr=${VC_DDR} \
            --link-width-bits=256 \
            --architecture-file="${GEM5}/my_benchmarks/11_25/Heterogeneous_SoC_with_Ring_Topology.arch"
