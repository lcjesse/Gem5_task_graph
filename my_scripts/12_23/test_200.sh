...
if [ "$1" = "run" ];then
            ...
            do
    {
            ...
            --task-graph-file="${GEM5}/my_benchmarks/12_23/Memory_Access_${MEM_ACCESS}_Memory_Type_${MEM_TYPE}/multi_application_0${APP_NUM}.cfg" \
            --routing-algorithm=2 \
            --vcs-per-vnet=2 \
            --link-width-bits=256 \
            --architecture-file="${GEM5}/my_benchmarks/12_23/Heterogeneous_SoC_with_Ring_Topology.arch"  &> ${OUT_FILE_DIR}/${DIR_NAME}/log  
