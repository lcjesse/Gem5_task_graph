...
OUT_FILE_DIR=${GEM5}/my_STATS/09_22_total_iters1
...
do
    ...
    do
    {
        ...
        --task-graph-file="./my_benchmarks/09_22/Application_0${i}.stp" \
        --execution-iteration=${j} \
        --routing-algorithm=2 \
        --vcs-per-vnet=2 \
        --link-width-bits=256 \
        --architecture-file="./my_benchmarks/09_22/Heterogeneous_SoC_with_Ring_Topology.arch"  &> ${OUT_FILE_DIR}/${DIR_NAME}/log
        #--ruby-clock=2GHz
        #--sys-clock=2GHz
