...
DATE=`date +%m_%d`
OUT_FILE_DIR=${GEM5}/my_STATS/${DATE}/time_core_id
...
do
            ...
            do
    {
        ...
        --task-graph-file="${GEM5}/my_benchmarks/10_12/Memory_Access_${MEM_ACCESS}_Memory_Type_${MEM_TYPE}/Application_0${APP_NUM}.stp" \
        --execution-iteration=${ITERS}\
        --routing-algorithm=2 \
        --vcs-per-vnet=2 \
        --link-width-bits=256 \
        --architecture-file="${GEM5}/my_benchmarks/10_12/Heterogeneous_SoC_with_Ring_Topology.arch" #\
